...
export default {
    props:(['StockCode', 'DealershipID', 'Dealer_Name', 'VehicleYear', 'VehicleMakeName', 'VehicleModelName', 'CurrentVehicle']),
    data: () => ({
        formisloading:false,
        nowtime:'',
        datetoday:'',
    ...
    methods:{
        ...
        async sendemail() {
          // let ouremail = this.DealershipID+'@autoslm.co.za'
          let ouremail = this.DealershipID+'@autoslm.co.za'
          let client = "info@secondhandcars.co.za"
          // let clientdealername = this.
          // var axios = require('axios');
            var data = JSON.stringify({
              "toaddress": ouremail,
               "ccaddress": client,
              "emailSubject": "Second Hand Cars Enquiry",
              "emailBody": "<div><h2>Good Day</h2><h4>You have a new enquiry from Second Hand Cars</h4><p>First Name: " + this.formdata.firstName +"</p><p>Last Name: " + this.formdata.lastName + "</p><p>Email: " + this.formdata.emailAddress + "</p><p>Mobile: " + this.formdata.contactNumber + "</p><p>Stockcode: " + this.StockCode + "</p><p>Vehicle: " + this.VehicleMakeName + " " + this.VehicleModelName + "</p><p>Comments: " + this.formdata.comments + "</p><p>Lead Source: Second Hand Cars</p><div style='background-image: url(https://secondhandcars.co.za/uploads/emailfooterbannernew1-01-min.png);width:600px;height:200px; background-size:cover; background-repeat: no-repeat'></div></div>"
            });
            //console.log('data', data)
